...
TARGET_BRANCH="main"
REPO_SLUG="sams-transportation/dsrc-repo"
...
for IND in "${IND_LIST[@]}"; do
  ...
  echo "Checking for diff between origin/$TARGET_BRANCH and origin/$SOURCE_BRANCH"
  if git diff --quiet origin/"$TARGET_BRANCH"..origin/"$SOURCE_BRANCH"; then
    echo "No changes for $IND — skipping PR."
    continue
  fi
